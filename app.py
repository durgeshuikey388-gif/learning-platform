...
# ================= POSTGRESQL DATABASE =================
DATABASE_URL = "postgresql://learningdb_m8ny_user:jcEG0MmXqPv3XaL7gktyEmcVLilGDWHI@dpg-d7uri01o3t8c73ft7hd0-a.ohio-postgres.render.com/learningdb_m8ny"
...
def course(course_id):
    ...
    c.execute("SELECT * FROM theory WHERE course=%s", (course_id,))
    data = c.fetchall()
...
def add_theory(course_id):
    ...
    c.execute(
        "INSERT INTO theory(course, content) VALUES (%s,%s)",
        (course_id, content)
    )
...
def delete(id, course):
    ...
    c.execute("DELETE FROM theory WHERE id=%s", (id,))
...
def download_theory(course_id):
    ...
    c.execute("SELECT content FROM theory WHERE course=%s", (course_id,))
    data = c.fetchall()
